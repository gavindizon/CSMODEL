...
class CollaborativeFiltering(object):
    ...
    def get_cosine_similarity(self, vector1, vector2):
        ...
        if(isinstance(vector1, pd.Series) and isinstance(vector2, pd.Series)):
            return ((vector1 * vector2).sum())/(((np.sqrt((vector1**2).sum())))*(np.sqrt(((vector2**2).sum()))))
        else:
            ...
            series = ((df * sr).sum(axis=1))/((np.sqrt((df**2).sum(axis=1))*(np.sqrt((sr**2).sum()))))
...
#            print(series)
    ...
    def get_k_similar(self, data, vector):
        ...
        # in this class
        cos_sim = self.get_cosine_similarity(normData, normVector)
        ...
        ksim = cos_sim.nlargest(n = self.k)        

        # TODO: Return 2 values. See function comment
        return [ksim.index, ksim]
    ...
    def get_rating(self, data, index, column):
        ...
        # class
        k_sim = self.get_k_similar(new_data, vector)


        # TODO: Compute for the rating using the similarity values and the raw
        # ratings for the k similar items.

        numerator = (new_data.iloc[:,column][k_sim[0].values].values * k_sim[1].values).sum()
        denominator = k_sim[1].values.sum()
